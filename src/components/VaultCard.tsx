...
export function VaultCard({ data }: { data: StaticVaultData }) {
  ...
  return (
    <div
      onClick={() =>
        !data.commingSoon && router.push(`/vault/${data.vaultAddress}`)
      }
      className={cn([
    ...
    >
      <div className='absolute top-0 left-0 right-0 flex justify-center translate-y-[-30px]'>
        <img
          width='60px'
          alt={`${data.token1} token icon`}
          src={tokenIcons[data.token1]}
          className='translate-x-[10px] group-hover:translate-x-[-3px] duration-150'
        />
        <img
          width='60px'
          alt={`${data.token2} token icon`}
          src={tokenIcons[data.token2]}
          className='translate-x-[-10px] group-hover:translate-x-[3px] duration-150'
        />
      </div>
      ...
      {data.commingSoon && (
        <div className='absolute text-xs right-0 top-0 border-light border-[3px] translate-y-[-50%] translate-x-5  bg-primary-600 rounded-full px-2 py-1'>
          Comming soon
        </div>
      )}
