...
export function DepositForm({
  ...
  const [token0Balance, setToken0Balance] = useState<TokenBalance>(
    formState.token0Balance
  );
  const [token1Balance, setToken1Balance] = useState<TokenBalance>(
  ...
  const onSubmit = useCallback((e: SyntheticEvent<HTMLFormElement>) => {
    ...
    const formState = {
      token0Balance: makeTokenBalanceFromBaseUnit(
        formData.token0Balance.toString(),
        vaultData.token0.token
      ),
      token1Balance: makeTokenBalanceFromBaseUnit(
        formData.token1Balance.toString(),
      ...
      <form onSubmit={onSubmit}>
        ...
        <div>
          <TokenInput
            user={user}
            name='token0Balance'
            updateTokenBalance={onBalance0Change}
            tokenBalance={token0Balance}
          />
          <TokenInput
            user={user}
            name='token1Balance'
            updateTokenBalance={onBalance1Change}
