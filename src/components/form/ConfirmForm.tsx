...
export function ConfirmForm({ formState, backToEdditing, confirm }: ConfirmFormProps) {
  const [confirmStage, setConfirmStage] = React.useState<ConfirmStage>(ConfirmStage.WaitingForConfirmation);
    ...
    <div>
      {
        confirmStage === ConfirmStage.WaitingForConfirmation && (
          <div>
            ...
            <Button onClick={onConfirm} variant="outline">Deposit</Button>
          </div>
        )
      }
      {
        confirmStage === ConfirmStage.WaitingForSignature && (
          <div>
            <h2>Accept transaction in your wallet!
            </h2>
            <p className="w-[70%] mb-4">
              You are about to deposit <span className="font-semibold text-primary-600">{formatTokenBalanceWithUnit(formState.token0Balance)}</span> and <span className="font-semibold text-primary-600">{formatTokenBalanceWithUnit(formState.token1Balance)}</span> into the Arrakis vault. Please review the transaction and confirm it.
            </p>
      ...
      }
      {
        confirmStage === ConfirmStage.TxPending && (
          <div>
            ...
            <p className="w-[70%] mb-4">
              You are about to depositing <span className="font-semibold text-primary-600">{formatTokenBalanceWithUnit(formState.token0Balance)}</span> and <span className="font-semibold text-primary-600">{formatTokenBalanceWithUnit(formState.token1Balance)}</span> into the Arrakis vault. Please wait for the <a href="">transaction</a> to be confirmed.
            </p>
          </div>
        )
      }
      {
        confirmStage === ConfirmStage.TxConfirmed && (
          <div>
            ...
            <p className="w-[70%] mb-4">
              You have just deposited <span className="font-semibold text-primary-600">{formatTokenBalanceWithUnit(formState.token0Balance)}</span> and <span className="font-semibold text-primary-600">{formatTokenBalanceWithUnit(formState.token1Balance)}</span> into the Arrakis vault. Thank you, now we take over from here so you can see your assets grow!
            </p>
          </div>
        )
      }
      {
        confirmStage === ConfirmStage.TxFailed && (
          <div>
            ...
            <p className="w-[70%] mb-4">
              Unfortunetly your transaction has failed. Please try again or contact support.
            </p>
            <Button className="mr-4" onClick={backToEdditing} variant="outline">Back to editing</Button>
            <Button onClick={onConfirm} variant="outline">Try Again</Button>
          </div>
        )
      }
    </div>
