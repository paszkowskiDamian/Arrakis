...
export function formatTokenBalanceWithUnit(tokenBalance: TokenBalance): string {
  return `${humanizeNumber(formatUnits(tokenBalance.balance, tokenBalance.token.decimals))} ${tokenBalance.token.symbol}`;
}
...
export function makeTokenBalanceFromBaseUnit(balance: string, token: Token): TokenBalance {
